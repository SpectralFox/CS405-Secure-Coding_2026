...
#include <tuple>
#include <vector>
#include <regex> // enables regular expression usage 
#include "sqlite3.h"
...
// clear any prior results
 //   records.clear();  // cancel out clearing results

// Returns true if the SQL string has generic injection patterns
    bool has_sql_injection(const std::string & sql); // helper  that prints an error if true, or return false;
{
//  Lowercase + remove quoted literals; checks for gneeric injection patterns 
    std::string clean; clean.reserve(sql.size()); 
    bool in_quote = false;
    for (char c : sql) 
    {
        if (c == '\'' && !in_quote) { in_quote = true; continue; }
        if (c == '\'' && in_quote) { in_quote = false; continue; }
        if (!in_quote) clean += std::tolower(c);
    }

//  Multiple semicolons → stacked statements
    if (std::count(clean.begin(), clean.end(), ';') > 1) 
    {
        return true;
    }

//  Generic tautologies, comment markers, UNION SELECT
    static const std::vector<std::regex> patterns = 
    {
        std::regex(R"(\bor\b\s+\w+\s*=\s*\w+)"),  // OR x = y
        std::regex(R"(;--)"),                     // inline comment
        std::regex(R"(/\*)"),                     // block comment start
        std::regex(R"(\bunion\b\s+select\b)"),    // UNION SELECT
    };

    for (auto& pat : patterns) 
    {
        if (std::regex_search(clean, pat)) 
        {
            return true;
        }
    }
